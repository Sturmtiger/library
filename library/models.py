...
class Book(models.Model):
    ...
    authors = models.ManyToManyField(
        "Author", related_name="books", through="BookAuthorsPriority"
    )
...
class Author(models.Model):
    ...
    def get_absolute_url(self):
        return reverse("library:author_detail", args=[self.slug])
...
class BookAuthorsPriority(models.Model):
    ...
    def save(self, *args, **kwargs):
        prev_same_book = BookAuthorsPriority.objects.filter(book=self.book).last()
        if prev_same_book:
